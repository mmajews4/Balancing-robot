    ...
    if (ESP_BT.connected()) {
        ...
        if(run){
            ...
            if(sendDelay == SENDING_PERIOD){
                sprintf(message, "P:%.0f   I:%.0f   D:%.0f\nAngle: %0.3f\nA+Str: %0.3f\nThrottle: %d", xP, xI, xD, angleX, currentValue, throttle);
            ...
            if(throttle != 0) time_to_next_step = MAX_SPEED_DELAY + ((200000/abs(motor_throttle))); // dobrać najmniejszą prędkość
...
void tune(){
    ...
    while(!run){
        ...
        take_measurement();
        sprintf(message, "P:%.0f   I:%.3f   D:%.0f\nAngle: %0.3f\nA+Str: %0.3f\nAdjst: %0.3f\n", vP, vI, vD, angleX, currentValue, adjustment);
        ESP_BT.print(message);
